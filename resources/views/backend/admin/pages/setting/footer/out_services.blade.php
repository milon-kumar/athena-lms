        ...
        <div class="col-md-12">
            <form class="form-horizontal" action="{{ route('admin.setting.update') }}" method="post"
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <input type="hidden" name="types[]" value="our_services">
                        <textarea name="our_services" id="ourServices" rows="20">{!! get_setting('our_services') !!}</textarea>
                    </div> <!-- end card-body -->
                </div> <!-- end card-->
            </form>
        </div> <!-- end col -->
...
@section('script')
    <script>
        ClassicEditor
            .create(document.querySelector('#ourServices'))
            .catch(error => {
                console.error(error);
            });
    </script>
